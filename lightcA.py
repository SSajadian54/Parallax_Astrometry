...
################################################################################
yr=float(364.0);
f0=open("./files/MONTA/IBH_MONT1a.dat","r")
...
for i in range(nr): 
    ...
    if(flagf>0 and icon>145): 
        f1=open("./files/MONTA/datA{0:d}.dat".format(icon),"r")
        nd=sum(1 for line in f1)  
        dat=np.zeros((nd,9)); 
        dat= np.loadtxt("./files/MONTA/datA{0:d}.dat".format(icon))
        
        
        f2=open("./files/MONTA/magA{0:d}.dat".format(icon),"r")
        nm=sum(1 for line in f2)  
        mag=np.zeros((nm,15));   
        mag= np.loadtxt("./files/MONTA/magA{0:d}.dat".format(icon))
        
        dat1=np.zeros((nd,9));   d1=0         
        dat2=np.zeros((nd,9));   d2=0
        for j in range(nd): 
            if(dat[j,8]>0):
                dat1[d1,:]= dat[j,:]## extra observation Roman
                d1+=1 
            if(dat[j,8]<0):
                dat2[d2,:]= dat[j,:]## common observation Roman
                d2+=1     
        ...
        plt.errorbar(dat1[:d1,0]/yr,dat1[:d1,3],yerr=dat1[:d1,4],fmt=".",markersize=6.5,color='darkred',ecolor='red',
        ...
        plt.title(r"$t_{\rm{E}}\rm{(days)}=$"+str(round(tE,1))+ r"$,~\theta_{\rm E}\rm{(mas)}=$"+str(round(tetE,2))+
          r"$,~\pi_{\rm E}=$"+str(round(piE,3))+r"$,~m_{\rm{bg},~W149}(\rm{mag})=$"+str(round(mbs,1)),fontsize=16, color="k")
        plt.legend()
        ax1.legend(prop={"size":15.5})
        fig=plt.gcf()
        fig.savefig("./lightc/1/lcA{0:d}.jpg".format(icon),dpi=200)
        ...
        plt.errorbar(dat1[:d1,5],dat1[:d1,6],yerr=dat1[:d1,7],xerr=dat1[:d1,7],fmt=".",markersize=5.5,color='darkred',ecolor='red',     
        ...
        plt.xticks(fontsize=17, rotation=0)
        plt.yticks(fontsize=17, rotation=0)
        plt.title(r"$M_{\rm{l}}(M_{\odot})=$"+str(round(Ml,1))+ r"$,~~D_{\rm{l}}\rm{(kpc)}=$"+str(round(Dl,1))+r"$,~~\pi_{\rm{rel}}=$"+str(round(pirel,3)),fontsize=18, color="k")
        ax1.legend(prop={"size":15.5})
        fig=plt.gcf()
        fig.savefig("./lightc/1/dfA{0:d}.jpg".format(icon),dpi=200)
        '''
        ######################################################################## 
        ...
        plt.title(r"$M_{\rm{l}}(M_{\odot})=$"+str(round(Ml,1))+ r"$,~D_{\rm{l}}\rm{(kpc)}=$"+str(round(Dl,1))+r"$,~\pi_{\rm{rel}}=$"+str(round(pirel,3))+r"$,~\theta_{\rm E}\rm{(mas)}=$"+str(round(tetE,2)),fontsize=19, color="k")
        ...
        fig.savefig("./lightc/1/dftA{0:d}.jpg".format(icon),dpi=200)
