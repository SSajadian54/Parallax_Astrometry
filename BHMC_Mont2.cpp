...
const double satu[M]={12.0, 12.0, 12.0, 13.0, 14.8}; //it should be changed
const double thre[M]={20.0, 21.0, 23.0, 21.0, 26.0};
const double FWHM[M]={0.33, 0.33, 5.0*0.004, 0.33 , 0.33};//3*pixel_size (0.11") of WFIRST, VIKH W149 Z087
const double AlAv[M]={1.009,0.600,0.118,0.184,0.225};///From besancon model[VIKH W149]
const double sigma[M]={0.022,0.022,0.04,0.025,0.025};//MOAاستفاده از مقاله کاردلی
//const double Akv=0.118;
const double cade1=double(15.16/60.0/24.0);//W149_cadence in  day
const double cade2=double(10.0); // in days
const double dt=cade1;//double(7.58/60.0/24.0);//days
const int coun= 37000 + int(double(Tobs*7.0/12.0)/cade2 +10);
...
double errELT(   roman & ro, double ghadr, int flag);
//////////////////////////////////////////////////////
    time_t _timeNow;
...
///===========================================================
    ...
    if(!elt){cout<<"cannot read sigma_WFIRST.txt: "<<endl; int yye;  cin>>yye;}
    for(int i=0; i<Nel;++i){
    fscanf(elt,"%lf   %lf   %lf\n",&ro.mage[i], &ro.ere1[i], &ro.ere2[i] );
    ro.ere2[i]=ro.ere2[i]*0.001;}//mili arcsec
    fclose(elt);
    cout<<"*****  sigma_ELT.txt was read *****  "<<endl;
...
///===========================================================
    FILE* fil1;
    fil1=fopen("./files/MONT1/IBH_MONT1b.dat","w");
    fclose(fil1);

    FILE* fil2;
    fil2=fopen("./files/MONT1/IBH_MONT2b.dat","w");
    fclose(fil2);
    
    FILE* fil3;
    fil3=fopen("./files/MONT1/IBH_MONT3b.dat","w");
    fclose(fil3);
    ...
    obs2[0][0]=0.0*year+0.0;    obs2[0][1]=0.0*year+float(7.0*30.5);
    obs2[1][0]=1.0*year+0.0;    obs2[1][1]=1.0*year+float(7.0*30.5);
    obs2[2][0]=2.0*year+0.0;    obs2[2][1]=2.0*year+float(7.0*30.5);
    obs2[3][0]=3.0*year+0.0;    obs2[3][1]=3.0*year+float(7.0*30.5);
    obs2[4][0]=4.0*year+0.0;    obs2[4][1]=4.0*year+float(7.0*30.5);
   

    as.tetp=double(M_PI/3.0);
    as.omegae=double(2.0*M_PI/year); ///radian per day
    as.vearth= as.omegae *1.01;  /// for L2 point from teh Sun Roman telescope 
    ...
    double timp1, timp2,lonn, chi, chi1, chi2;
    double chi1a, chi2a, dchia, def, def0, sil; ///*** 
    float  flag0, flag1, flag2, fel;
    double prob1, prob2; 
    double dchi, dchi2,  mh;
    ...
    float  sig[2]={+1.0, -1.0};  
    double errg, flago1, numc,flago2,amp, errs, deltaA; 
    int    datf1, datf2;
    int nsim=0; 
    int sea1, sea2; 
   ...
   int    *tele=new int [coun];  
     ...
     func_source(s, cm, ex);
     func_lens(l, s);
     }while(l.tE<=0.1 or l.tE>2000.0);// which is longer than 5 years
     optical_depth(s);
     ...
     if(test<=s.fb[0] and s.mbs[0]<=thre[4]){///Roman should discern first 
     ...
     if(counter<1){
     flagf=1;
     sprintf(filnam1,"./files/MONT1/%c%c%c%c%d.dat",'m','a','g','B', counter);
     sprintf(filnam2,"./files/MONT1/%c%c%c%c%d.dat",'d','a','t','B', counter);
...
///*********************************************************************************   
     ...
     timp1=0.0;  timp2=0.0;  fel=0.0;
     s.ampM=0.0; s.ampA=0.0; s.errM=14630000000.0;  s.errA=1164333359450.0; 
     numc=0.0 ;  
     
     
     
     for(double tim=float(-7.0*year);  tim<float(12.0*year); tim=tim+dt){
     ...
     //amp= sqrt((s.def1a-s.def1c)*(s.def1a-s.def1c) + (s.def2a-s.def2c)*(s.def2a-s.def2c))/sqrt(s.def1a*s.def1a+s.def2a*s.def2a); 
     //if(amp>s.ampA)  s.ampA= amp;  
     //amp= fabs(s.Astar -Astar0)/Astar0; /// amplitude of magnification curve 
     //if(amp>s.ampM)   s.ampM=amp;  
     ...
     sea1=-1; 
     sea2=-1;
     for(int kl=0; kl<6; kl=kl+1){
     if(float((obs1[kl][0]-tim)*(obs1[kl][1]-tim))<=0.0 and tim>0.0 and tim<Tobs ){flago1=1.0; sea1=kl;  break;}}
     for(int kl=0; kl<5; kl=kl+1){
     if(float((obs2[kl][0]-tim)*(obs2[kl][1]-tim))<=0.0 and tim>0.0 and tim<Tobs ){flago2=1.0; sea2=kl;   break;}}
     
     
     
     datf1=0; 
     if(flago1>0.5){//Roman observation 
     timp1 += dt; 
     ...
     datf2=0;
     if(flago2>0.5){//ELT observation
     timp2 += dt; 
     if(timp2>cade2 or timp2==cade2){
     timp2-=cade2;
     ...
     if(datf1>0 and magni[4]>=satu[4] and magni[4]<=thre[4]){//Roman data
     errg= errwfirst(ro, magni[4], 0); 
     errs= errwfirst(ro, magni[4], 1); 
     magnio=magni[4] + RandN(errg ,3.0);
     chi  +=fabs( (magnio- s.magb[4])*(magnio- s.magb[4])/(errg*errg));
     ...
     tele[ndw]=0; 
     flag2=0.0;
     if(fabs(magnio-s.magb[4])>fabs(4.0*errg))    flag2=1.0;
     ...
     //s.errM += deltaA;  
     //s.errA += errs; 
     if(flagf>0) fprintf(data3,"%.4lf  %.4lf  %.6lf  %.6lf  %.4lf  %.4lf   %.4lf  %.6lf  %.1f  %d\n",
     tim,magnio,errg,s.Astar+RandN(deltaA,3.0),deltaA, s.def1c+RandN(errs,3.0), s.def2c+RandN(errs,3.0), errs, flago1, sea1);
     if(sea1<0 or sea1>5 or flago1<0.5) {cout<<"Error    sea2:  "<<sea1<<endl;  int yys; cin>>yys;}
     ndw+=1; }
     ...
     if(datf2>0 and fel>0.0 and magni[2]>=satu[2] and magni[2]<=thre[2]){//ELT data
     errg= errELT(ro, magni[2], 0); 
     errs= errELT(ro, magni[2], 1);      
     magnio=magni[2] + RandN(errg ,3.0);
     chi  +=fabs( (magnio- s.magb[2])*(magnio- s.magb[2])/(errg*errg));
     chi1 +=fabs( (magnio-  magni[2])*(magnio-  magni[2])/(errg*errg));
     chi2 +=fabs( (magnio- magni0[2])*(magnio- magni0[2])/(errg*errg)); 
     sil=  RandN(errs*sqrt(2.0), 3.0) ;
     chi1a+= fabs( (def-def0)*(def-def0)/(errs*errs*2.0) );
     chi2a+= fabs( (def+sil-def)*(def+sil-def)/(errs*errs*2.0) );
     timn[ndw]=tim;
     ...
     //s.errA += errs;
     if(flagf>0) fprintf(data3,"%.4lf  %.4lf  %.6lf  %.6lf  %.4lf  %.4lf   %.4lf  %.6lf %.1f   %d\n",
     tim,magnio,errg,s.Astar+RandN(deltaA,3.0),deltaA, s.def1c+RandN(errs,3.0), s.def2c+RandN(errs,3.0), errs,-flago2, sea2 );
     if(sea2<0 or sea2>4 or flago2<0.5) { cout<<"Error    sea2:  "<<sea2<<endl;  int yys; cin>>yys;  }  
     ndw+=1;} 
     if(ndw>=coun){cout<<"Error ndw:  "<<ndw<<"\t coun:  "<<coun<<endl;  int uue; cin>>uue;} }//loop of time
    ...
    ///************ WFIRST *****************************************************
     ...
     fil1=fopen("./files/MONT1/IBH_MONT1b.dat","a+");
     fprintf(fil1,
   "%d  %.5lf   %.5lf  "///3
   "%d  %.5lf   %.5lf   %.5lf  "///7
   "%d   %d  %.5lf   %.5lf  %.4lf  %.4lf  %.4lf  %.2lf  %.3lf  %.4lf  %.4lf   %.4lf   %.4lf  %.4lf  "   //21
   "%.5lf  %.5lf  %.4lf  %.4lf   %.2lf  %.2lf   %.4lf  %.4lf   "  //29
   "%.6lf  %.5lf  %.5lf  %.7lf    %.5lf  %.5lf  %.4lf  %.6lf  %.9lf " ///38
   "%d  %d %.1lf  %d   %d   %.8lf  %.8lf   %.5lf  %.7lf  %.7lf   %.7lf   %.8lf   %.8lf   %.8lf   %.8lf  %.5lf  %.5lf\n", //55
   counter,s.lat, s.lon, //3
   l.struc, l.Ml, l.Dl, l.vl, //7
   s.struc, s.cl, s.mass, s.Ds, s.Tstar, s.Rstar, s.logl, s.type, s.col, l.vs, s.Mab[4], s.Mab[2], s.Map[4], s.Map[2],//21
   s.mbs[0],s.mbs[1], s.fb[0], s.fb[1], s.Nblend[4], s.Nblend[2], s.Ai[4], s.Ai[2], //29
...
///######################################################  
   ...
   co.FishM[j][k] += derm1f*derm2f/(errm[i]*errm[i]);}//end of k
   ...
   fil2=fopen("./files/MONT1/IBH_MONT2b.dat","a+");
...
///HHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH   
   ...
   fil3=fopen("./files/MONT1/IBH_MONT3b.dat","a+");
...
///HHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH  
   ...
   delete [] magn, timn, soux, souy, erra, errm, tele; 
   return(0);
}
///&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
double RandR(double down, double up){
...
double errELT(roman & ro, double ghadr, int flag){
     ...
     if(ghadr<ro.mage[0] or  ghadr==ro.mage[0])       error= ro.ere1[0];
     
     else if(ghadr>ro.mage[Nel-1] or ghadr==ro.mage[Nel-1]){
     shib=(ro.ere1[Nel-1]-ro.ere1[Nel-2])/(ro.mage[Nel-1]-ro.mage[Nel-2]); 
     error=ro.ere1[Nel-1]+shib*(ghadr-ro.mage[Nel-1]);     }
     
     else{
     for(int i=1; i<Nel; ++i){
     if(double((ghadr-ro.mage[i])*(ghadr-ro.mage[i-1]))<0.0 or  ghadr==ro.mage[i-1]){
     shib=(ro.ere1[i]-ro.ere1[i-1])/(ro.mage[i]-ro.mage[i-1]); 
     error=ro.ere1[i-1]+shib*(ghadr-ro.mage[i-1]); 
     break;}}} 
     }
     ...
     if(ghadr<ro.mage[0] or  ghadr==ro.mage[0])            error= ro.ere2[0];
      
     else if(ghadr>ro.mage[Nel-1] or ghadr==ro.mage[Nel-1]){
     shib=(ro.ere2[Nel-1]-ro.ere2[Nel-2])/(ro.mage[Nel-1]-ro.mage[Nel-2]); 
     error=ro.ere2[Nel-1]+shib*(ghadr-ro.mage[Nel-1]); }   

     else{
     for(int i=1; i<Nel; ++i){
     if(double((ghadr-ro.mage[i])*(ghadr-ro.mage[i-1]))<0.0 or ghadr==ro.mage[i-1]){
     shib=(ro.ere2[i]-ro.ere2[i-1])/(ro.mage[i]-ro.mage[i-1]);
     error=ro.ere2[i-1] +  shib*(ghadr-ro.mage[i-1]); 
     break;}}}
     }
...
void func_source(source & s, CMD  &  cm, extinc & ex)
{
    ...
    for(int i=0; i<M; ++i){s.magb[i]=0.0; s.Ai[i]=0.0; s.Map[i]=0.0; s.Mab[i]=0.0;}
